...
export const registerSeller = asyncHanlder(async (req, res, next)=>{
    ...
    sentTokenToClient('sellerAccToken', accessToken, {
        httpOnly: true, 
        secure: process.env.NODE_ENV === "production",
        sameSite: true
    });
    sentTokenToClient('sellerRefToken', refreshToken, {
        httpOnly: true, 
        secure: process.env.NODE_ENV === "production",
        sameSite: true
    });
...
export const loginSeller = asyncHanlder(async (req, res, next)=>{
    ...
    if(!isPasswordMatch)return next(new ErrorHandler(400, "Incorrect password"));
    // Generate Tokens 
